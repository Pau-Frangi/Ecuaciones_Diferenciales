...
\ejercicio{medio}{profesor}{
    ...
    Resolvamos cada apartado:
    \begin{enumerate}
        \item[(a)] En este caso, el operador diferencial es $P(D) = (D - \lambda)(D - \mu)$, el polinomio característico asociado al homogéneo es $P(z) = (z - \lambda)(z - \mu)$ cuyas raices son $\lambda$ y $\mu$, por lo que deducimos que una base para el conjunto de soluciones del homogéneo asociado es:
        \[
            B = \{e^{\lambda t}, e^{\mu t}\} \, \qquad \text{base de } \Sigma_0
        \]
        Para sacar la solución particular, aplicamos el método de variación de coeficientes, proponiendo $u(t) = v(t) e^{\lambda t}$ y nos deja:
        \[
            \begin{cases}
                v' e^{\lambda t} = 0 \\
                v' e^{\mu t} = e^{\omega t}
            \end{cases} \implies
            \lambda^2 e^{\lambda t} v + 2 \lambda e^{\lambda t} v' + e^{\lambda t} v'' - (\lambda + \mu) \lambda e^{\lambda t} v - (\lambda + \mu) e^{\lambda t} v' + \lambda \mu e^{\lambda t} v = e^{\omega t}
        \]
        \[
            v' \left( 2 \lambda e^{\lambda t} - (\lambda + \mu) e^{\lambda t} \right) + v'' e^{\lambda t} = e^{\omega t} \implies
            v'' + (\lambda - \mu) v' = e^{(\omega - \lambda) t}
        \]
        Otra forma de hacerlo es buscar la solución general de esta ecuación diferencial con coeficientes constantes:
        \[
            u(t) = e^{\lambda t} x + e^{\mu t} y + p(t)
        \]
        Donde $p(t)$ es una solución particular de la ecuación diferencial anterior, que podemos encontrar usando que:
        \[
        p(t) = m e^{\omega t} \implies
        u = \frac{e^{\omega t}}{P(D)}
        \]
        Dejando la solución particular como:
        \[
            p(t) = \frac{e^{\omega t}}{(\omega - \lambda)(\omega - \mu)}
        \]
    \end{enumerate}
}
