...
\begin{definición}[Ecuación de Bernoulli]
...
\subsection{La Ecuación de Airy}

Otro ejemplo clásico e importante en física matemática es la \textbf{ecuación de Airy} (formulada en 1838), la cual aparece al estudiar ciertos fenómenos de difracción y mecánica cuántica:
\[
    u''(z) - z u(z) = 0
\]
A diferencia de la ecuación de Bessel, el punto $z=0$ es un punto ordinario, por lo que podemos proponer directamente un desarrollo en serie de potencias clásico:
\[
    u(z) = \sum_{n=0}^{\infty} u_n z^n
\]
Calculamos la segunda derivada e introducimos la serie en la ecuación diferencial:
\[
    \sum_{n=2}^{\infty} n(n-1)u_n z^{n-2} - z \sum_{n=0}^{\infty} u_n z^n = 0 \implies \sum_{n=2}^{\infty} n(n-1)u_n z^{n-2} - \sum_{n=0}^{\infty} u_n z^{n+1} = 0
\]
Para agrupar ambas sumas bajo la misma potencia de $z$, realizamos cambios de índice de modo que el exponente general sea $z^{m+1}$:
\begin{itemize}
    \item En la primera suma: $n-2 = m+1 \implies n = m+3$. Como $n$ empieza en $2$, el nuevo índice $m$ empezará en $-1$.
    \item En la segunda suma: $n = m$. El índice $m$ empezará en $0$.
\end{itemize}
Sustituyendo estos índices, la ecuación se reescribe como:
\[
    \sum_{m=-1}^{\infty} (m+3)(m+2)u_{m+3} z^{m+1} - \sum_{m=0}^{\infty} u_m z^{m+1} = 0
\]
Extraemos el término solitario correspondiente a $m=-1$ de la primera suma para poder unificar los sumatorios desde $m=0$:
\[
    (-1+3)(-1+2)u_2 z^0 + \sum_{m=0}^{\infty} \Big[ (m+3)(m+2)u_{m+3} - u_m \Big] z^{m+1} = 0
\]
Simplificando el término independiente, obtenemos $2 u_2 = 0$, lo que implica obligatoriamente que:
\[
    u_2 = 0
\]
Para el resto de términos ($m \geq 0$), igualamos a cero el coeficiente de $z^{m+1}$, obteniendo la \textbf{relación de recurrencia} entre los coeficientes:
\[
    (m+3)(m+2)u_{m+3} - u_m = 0 \implies u_{m+3} = \frac{u_m}{(m+3)(m+2)}, \quad \forall m \geq 0
\]
Esta relación de recurrencia da saltos de tres en tres, lo que divide a los coeficientes en tres sucesiones independientes:
\begin{itemize}
    \item $u_2 \implies u_5 \implies u_8 \implies \dots \implies u_{3k+2} = 0$. Todos los términos de esta familia se anulan.
    \item $u_0 \implies u_3 \implies u_6 \implies \dots \implies u_{3k}$.
    \item $u_1 \implies u_4 \implies u_7 \implies \dots \implies u_{3k+1}$.
\end{itemize}
Resolvamos la sucesión para los coeficientes $u_{3k}$ en función de $u_0$. Aplicando la recurrencia:
\[
    u_{3k} = \frac{u_{3(k-1)}}{3k(3k-1)}
\]
Si multiplicamos numerador y denominador por el factor faltante $(3k-2)$ para conseguir el factorial en el denominador:
\[
    u_{3k} = \frac{(3k-2) u_{3(k-1)}}{3k(3k-1)(3k-2)} = \dots = \frac{\prod_{j=0}^{k-1} (3j+1)}{(3k)!} u_0
\]
Análogamente, para los coeficientes $u_{3k+1}$ dependientes de $u_1$, multiplicamos por $(3k-1)$:
\[
    u_{3k+1} = \frac{u_{3(k-1)+1}}{(3k+1)(3k)} = \frac{(3k-1) u_{3(k-1)+1}}{(3k+1)(3k)(3k-1)} = \dots = \frac{\prod_{j=0}^{k-1} (3j+2)}{(3k+1)!} u_1
\]
De este modo, hemos encontrado las soluciones en serie de potencias que originan las \textbf{Funciones de Airy}. Tomando la familia asociada a $u_0$ (con $u_0 = 1$), su desarrollo analítico es:
\[
    \text{Airy}(z) = \sum_{k=0}^{\infty} u_{3k} z^{3k} = \sum_{k=0}^{\infty} \frac{\prod_{j=0}^{k-1} (3j+1)}{(3k)!} z^{3k}
\]