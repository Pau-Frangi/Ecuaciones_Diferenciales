...
\ejemplo{
    \textbf{Identificación de una ecuación no exacta} \\
    Consideremos el siguiente problema diferencial:
    \[
        u' = -\frac{u^2 + tu}{t^2}
    \]
    Para evaluar si podemos resolverla mediante potenciales, la reordenamos multiplicando en cruz para llevarla a su forma diferencial canónica:
    \[
        (u^2 + tu) + t^2 u' = 0
    \]
    Identificamos de inmediato los coeficientes $P(t,u) = u^2 + tu$ y $Q(t,u) = t^2$. Para aplicar el Teorema de Poincaré, procedemos a calcular sus derivadas cruzadas:
    \begin{align*}
        \frac{\partial P}{\partial u} &= \frac{\partial}{\partial u} (u^2 + tu) = 2u + t \\
        \frac{\partial Q}{\partial t} &= \frac{\partial}{\partial t} (t^2) = 2t
    \end{align*}
    Como es evidente, $\frac{\partial P}{\partial u} \neq \frac{\partial Q}{\partial t}$ en casi todo el dominio $\mathbb{R}^2$ (las derivadas solo coinciden en la recta $u = t/2$). El teorema nos garantiza, por tanto, que es material y topológicamente \textbf{imposible} construir una función potencial global $V(t,u)$ que satisfaga la ecuación. 
    
    En consecuencia, declaramos que \textbf{no es una ecuación diferencial exacta} en su forma actual, y nos veríamos obligados a recurrir a técnicas alternativas (como la búsqueda de un "factor integrante" adecuado que la fuerce a ser exacta, o tratarla como una ecuación de tipo homogéneo debido al grado de sus variables) para lograr resolverla.
}