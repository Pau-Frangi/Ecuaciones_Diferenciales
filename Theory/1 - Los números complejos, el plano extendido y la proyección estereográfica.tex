...
\begin{teorema} [Teorema de Existencia y Unicidad General]
    Para cada $t_0 \in [\alpha, \beta]$ y cada vector inicial $u_0 \in \mathbb{K}^n$, el problema de Cauchy:
    \[
        \begin{cases}
        u' = A(t) u + B(t), \quad t \in [\alpha, \beta] \\
        u(t_0) = u_0
        \end{cases}
    \]
    tiene una \textbf{única} solución $u \in C^{1}([\alpha, \beta], \mathbb{K}^n)$.
\end{teorema}
...
Como consecuencia de este teorema, se puede demostrar que $\dim(\Sigma_0) = n$ y, por tanto, $\Sigma_B$ es una variedad afín de dimensión $n$.
...
\ejemplo{
    ...
    \[
        p(t) = me^{at} 
    \]
    como es solución de la homogénea, no puede ser solución del inhomogéneo. Probamos con:
}