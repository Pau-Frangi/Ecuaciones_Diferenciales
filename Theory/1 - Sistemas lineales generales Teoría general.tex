...
\begin{teorema}[Fundamental global de Cauchy-Lipschitz]
    Sean $N \geq 1$, $\alpha > \beta$, y $f \in C([\alpha, \beta] \times \mathbb{R}^N, \mathbb{R}^N)$ una función globalmente Lipschitziana respecto a $u \in \mathbb{R}^N$, uniformemente en $t \in [\alpha, \beta]$, es decir, $\exists L \geq 0$ tal que:
    \[
    ...
    \[
        ...
        \begin{cases}
            ...
            u(t_0) = u_0
        \end{cases}
        \underbrace{=}_{\text{equivalentes, notación}}
        \begin{cases}
            u' = f(t, u), \quad t \in [\alpha, \beta] \\
            u(t_0) = u_0
        \end{cases}
    \]
    Tiene una única solución globalmente definida en todo el intervalo $[\alpha, \beta]$, denotada por:
    \[
        \boxed{u(t) = u(t; t_0, u_0), \quad t \in [\alpha, \beta]
        }
    \]
\end{teorema}
...
\begin{proof}
    Sea:
    \[
        u \in C^1([\alpha, \beta]; \mathbb{R}^N) \longrightarrow
        \begin{cases}
            u'(t) = f(t, u(t)), \quad t \in [\alpha, \beta] \\
            u(t_0) = u_0
        \end{cases}
    \]
    Por lo visto en anteriores asignaturas:
    \[
        u \text{ es solución}
        \iff
        u \in C([\alpha, \beta]; \mathbb{R}^N) : \quad
        u(t) = u_0 + \int_{t_0}^t f(s, u(s)) ds, \quad \forall t \in [\alpha, \beta]
    \]
    Ya que integrando $u'= f(t,u)$ desde $t_0$ hasta $t$ obtenemos:
    \[
        \int_{t_0}^t u' = \int_{t_0}^t f(s, u(s)) ds \implies
        u(t) - u(t_0) = \int_{t_0}^t f(s, u(s)) ds \implies
        u(t) = u_0 + \int_{t_0}^t f(s, u(s)) ds
    \]
    Una vez recordado esto, definimos el operador integral:
    \[
        \begin{array}{lll}
            K : C([\alpha, \beta]; \mathbb{R}^N) & \longrightarrow & C([\alpha, \beta]; \mathbb{R}^N) \\
            u & \longmapsto & (Ku)(t) = u_0 + \int_{t_0}^t f(s, u(s)) ds
        \end{array}
    \]
    Siendo las soluciones del problema $P$ los puntos fijos de $K$. Como tenemos que $C([\alpha, \beta]; \mathbb{R}^N)$ es un espacio de Banach con la norma del supremo, que es equivalente a la norma de Bielecki:
    \[
        \| u \|_B = \max_{t \in [\alpha, \beta]} e^{-L|t - t_0|} \| u(t) \|
    \]
\end{proof}